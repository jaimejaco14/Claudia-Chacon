...
if($id = $_POST['nombre']) {
      ...
      if ($_POST['estado'] != ''){ 
          $sql = $sql." AND bty_fnc_estado_colaborador(c.clbcodigo) = '".$_POST['estado']."'"; 
      } 
      if ($_POST['vinculacion'] != ''){ 
           $sql="SELECT c.clbcodigo, t.trcrazonsocial, t.trcdocumento, cr.crgnombre, ct.ctcnombre, c.cblimagen, td.tdialias, ct.ctccolor, ct.ctccodigo, cr.crgalias
                  FROM btycolaborador AS c
                  join btyestado_colaborador ec on ec.clbcodigo=c.clbcodigo
                  JOIN btycolaborador_vinculacion cv ON cv.clecodigo=ec.clecodigo, btytercero AS t, btytipodocumento AS td, btycargo AS cr, btycategoria_colaborador AS ct
                  WHERE c.trcdocumento=t.trcdocumento AND c.tdicodigo=t.tdicodigo AND td.tdicodigo=c.tdicodigo AND cr.crgcodigo=c.crgcodigo AND ct.ctccodigo=c.ctccodigo AND c.clbestado='1' AND bty_fnc_estado_colaborador(c.clbcodigo) = 'VINCULADO' AND cv.tivcodigo = ".$_POST['vinculacion'];
      } 
      if ($_POST['cargo'] != 0 ) { 
      ...
      if ($_POST['salonBase'] != 0 ) { 
          $sql = $sql." AND bty_fnc_salonbase_col(c.clbcodigo) = ".$_POST['salonBase']; 
      } 
      $filter=strlen($_POST['estado'])+strlen($_POST['vinculacion'])+$_POST['cargo']+$_POST['perfil']+$_POST['salonBase'];
...
//echo $sql;
  ...
  if($filter>0){
    echo '<a class="resetfilter btn btn-primary pull-left"><i class="fa fa-filter"></i> Quitar filtro</a>';
  }
  ...
  if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($exists == true) {    
            $file = "<a href='".$rut."?ts=".rand(1000,9999)."' target='_blank' title='Ver RUT del colaborador' ><i class='fa fa-file-text-o'></i></a>";
        } else {
            $file = "";
        }
        ...
        if ($row['cblimagen'] != "default.jpg") {
            $img = "colaborador/".$_SESSION['Db']."/".$row['cblimagen']."?ts=".rand(1000,9999);
        }else{
            $img = $row['cblimagen'];
        }
    ...
    echo '<h3><i class="fa fa-times text-danger"></i> No hay resultados</h3>';
  }
