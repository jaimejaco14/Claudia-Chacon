            ...
            <form >     
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="vincu form-group col-lg-3 hidden">
                            <label>Tipo de Vinculación</label>
                            <select id="FiltroVinculacion" class="form-control">
                            <?php
                                $sql = "SELECT tivcodigo, tivnombre FROM btycolaborador_tipo_vinculacion";
                                $result = $conn->query($sql);
                                if ($result->num_rows > 0) {
                                    echo "<option value=''>--Todos--</option>";
                                    while ($row = $result->fetch_assoc()) {
                                        echo "<option value='".$row['tivcodigo']."'>".utf8_encode($row['tivnombre'])."</option>";
                                    }
                                }
                            ?>
                            </select>
       ...
       var arr1 = [];
        ...
        function filtrar () {
            $.ajax({
                type: "POST",
                url: "find.php",
                data: {cargo: $('#FiltroCargo').val(), perfil: $('#FiltroPerfil').val(), salonBase: $('#FiltroSalon').val(), estado: $('#FiltroEstado').val(), vinculacion: $('#FiltroVinculacion').val()},
                success: function (res) {
                    $('#find').html(res);
                }
            });
        ...
        $('#FiltroEstado').change(function() {
            var val=$(this).val();
            if(val=='VINCULADO'){
                $(".vincu").removeClass('hidden');
            }else{
                $("#FiltroVinculacion").val('');
                $(".vincu").addClass('hidden');
            }
            filtrar();
        });
        $('#FiltroVinculacion').change(function() {
            filtrar();
        });
...
<script>
    ...
    function detalles(id) {
        $.ajax({
            ...
            alert(e+',Error al cargar modulo');
        });
    }
    function verifultact(clb){
        $.ajax({
            ...
            success:function(res){
                ...
                $(".infogen-alt").html(cdp);
                $(".saludbi-alt").html(cs);
                $(".infofami-alt").html(cif);
                if(dat.cdp!=null && dat.cif!=null){
                    $(".btndatagen").removeClass('hidden');
                }else{
                    $(".btndatagen").addClass('hidden');
                }
            }
        })
    }

    function catpend(clb){
        $.ajax({
            url:'actualizaciondatos/process.php',
            type:'POST',
            data:{opc:'catpend',clb:clb},
            success:function(res){
                var rst=JSON.parse(res);
                if(rst.cdp==1){$('#infogen span').addClass('text-danger');}else{$('#infogen span').removeClass('text-danger')}
                if(rst.cs==1){$('#saludbi span').addClass('text-danger');}else{$('#saludbi span').removeClass('text-danger')}
                if(rst.cif==1){$('#infofami span').addClass('text-danger');}else{$('#infofami span').removeClass('text-danger')}
            }
        })
    }
    ...
    function paginar(id) {
        $.ajax({
            type: "POST",
            url: "find.php",
            data: {cargo: $('#FiltroCargo').val(), perfil: $('#FiltroPerfil').val(), salonBase: $('#FiltroSalon').val(), estado: $('#FiltroEstado').val(), page: id},
        }).done(function (a) {
            $('#find').html(a);
        }).fail(function () {
            alert('Error al cargar modulo');
...
<script type="text/javascript">
    $(document).ready(function() {    
        ...
        $("#btnReporteExcel").on("click", function(){
            var dato={cargo: $('#FiltroCargo').val(), perfil: $('#FiltroPerfil').val(), salonBase: $('#FiltroSalon').val()};
            window.open("./generarReporteColaboradores.php?dato=" + JSON.stringify(dato) + "&tipoReporte=excel");
        });

        $("#btnReportePdf").on("click", function(){
            var dato={cargo: $('#FiltroCargo').val(), perfil: $('#FiltroPerfil').val(), salonBase: $('#FiltroSalon').val()};
            window.open("./generarReporteColaboradores.php?dato=" + JSON.stringify(dato) + "&tipoReporte=pdf");
        });
    });    
</script>
