                ...
                <div class="hpanel">
                    ...
                    <div class="panel-body">
                       <!--  <button type="button" class="btn btn-info btn-xs pull-right"  data-toggle="modal" data-target="#modalFiltrocargo" id="btn_filtro_col" title="Ver colaboradores" style="margin-left: 10px"><i class="fa fa-plus-square"></i></button> --> 
                        <button type="button" class="btn btn-primary btn-xs pull-right" id="btnVista" data-toggle="tooltip" data-placement="right" title="Vista general de programación"><i class="fa fa-search"></i></button>
                         <button type="button" class="btn btn-warning btn-xs pull-right" id="btnNewProg" data-toggle="tooltip" data-placement="top" title=" Programación Rápida" style="margin-right: 10px"><i class="fa fa-users"></i></button>
                         <button type="button" class="btn btn-danger btn-xs pull-right" id="btnNewProg2" data-toggle="tooltip" data-placement="top" title=" Programación Rápida 2" style="margin-right: 10px"><i class="fa fa-users"></i></button>
...
<div class="modal fade" id="modalFiltrocargo" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h6 class="modal-title" id="txt_title">Colaboradores Salón </h6>
          </div>
          <div class="modal-body">           
            <div class="row">
                <div class="col-md-12">               
...
<!-- Modal -->
<div class="modal fade" id="modalEliminarProRango" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel"><i class="fa fa-times"></i> Eliminar programación</h4>
      </div>
      <div class="modal-body">
            <form action="" method="POST" role="form">  
                <input type="hidden" id="clbcodigoDel">       
              <div class="row">
                    <div class="col-md-6">
                         <div class="form-group">
                             <label for="">Fecha Inicial</label>
                             <input type="text" class="form-control" id="txtFechaInicial" placeholder="Fecha inicial">
                         </div>                         
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                             ...
                             <input type="text" class="form-control" id="txtFechaFinal" placeholder="Fecha final">
                         </div>
                    </div>
                    <div class="col-md-12">
                        <div id="informe"></div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
        <button type="button" class="btn btn-info" id="btnEliminarProgRan"><i class="fa fa-times"></i> Eliminar</button>
      </div>
    </div>
  </div>
</div>
...
$(document).on('click', '#btnNewProg', function() 
{
   window.location="aaprog.php";
});
$(document).on('click', '#btnNewProg2', function() 
{
   window.location="prg_col.php";
});
    ...
    $.ajax({
        ...
        success: function (data) 
        {
            ...
            $('#tbl_filter_col tbody').empty();
                ...
                for(var i in array)
                {                   
                    $('#txt_title').html("<i class='fa fa-users'></i> COLABORADORES: " +salon);
...
$(document).ready(function() {
     $(document).on('click', '#btnReporteMes', function() {
        ...
        window.open("generarReporteTurnosMes.php?salon="+$("#selectSalon").val()+"&fecha="+$('#fecha').val()+"&nomsalon="+nomsalon+"");        
    });
...
$(document).on('click', '#btnDelRango', function() 
{
    var clbcodigo = $(this).data("clbcodigo");
    $('#clbcodigoDel').val(clbcodigo);
    $('#modalEliminarProRango').modal("show");
});

$('#modalEliminarProRango').on('show.bs.modal', function () {
...
$('#txtFechaInicial').on('changeDate', function(ev)
{
...
$(document).on('click', '#btnEliminarProgRan', function() 
{
    var clbcod = $('#clbcodigoDel').val();
    var fechai = $('#txtFechaInicial').val();
    var fechaf = $('#txtFechaFinal').val();

    $.ajax({
        url: 'eliminarProRango.php',
        method: 'POST',
        data: {opcion: "eliminarPro", clbcod:clbcod, fechai:fechai, fechaf:fechaf},
        success: function (data) 
        {
            if (data == 1) 
            {
                swal("Se ha eliminado la programación", "", "success");
                $('#modalEliminarProRango').modal("hide");
            }
        }
    });
});
...
function loadProgramacion () 
{
    ...
    $.ajax({
        url: 'eliminarProRango.php',
        method: 'POST',
        data: {opcion: "loadData", clbcod:clbcod, fechai:fechai, fechaf:fechaf},
        success: function (data) 
        {
            ...
            if (json.res == "full") 
            {
                ...
                {
                    $('#informe').html("ELIMINAR PROGRAMACIÓN DE <b>" + json.json[i].nombre + "</b> ENTRE EL " + fechai + " Y " + fechaf);
                }
            }
        }
    });
