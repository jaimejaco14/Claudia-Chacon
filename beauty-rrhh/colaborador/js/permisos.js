...
$('#tbl_permisos_lista tbody').on('click', '#btnAnular', function() 
{
        var $row = $(this).closest("tr");    // Find the row
        var $id = $row.find(".sorting_1").text(); // Find the text
        var cod = $id;
        ...
        function(isConfirm){
          if (isConfirm) {
            $.ajax({
                url: 'eliminarPermiso.php',
                method: 'POST',
                data: {cod:cod, opcion: "anular"},
                success: function (data) {
                    if (data == 1) {
                        swal("Anulado!", "Permiso Anulado", "success");
                        tbl_permisos();
                    }
                }
            });
