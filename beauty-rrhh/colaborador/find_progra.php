...
if ($_GET['p']== "salon") {
	...
	$sql = "SELECT DISTINCT CONCAT(p.prgfecha,' ',t.trndesde) start, CONCAT(p.prgfecha,' ',t.trnhasta) end, CASE WHEN tpr.tprnombre = 'DESCANSO' THEN 'DESCANSO' WHEN tpr.tprnombre = 'PERMISO' THEN 'PERMISO' WHEN tpr.tprnombre = 'CAPACITACION' THEN 'CAPACITACION' WHEN tpr.tprnombre = 'LABORA' THEN 'LABORA' WHEN tpr.tprnombre = 'META' THEN 'META' WHEN tpr.tprnombre = 'INCAPACIDAD' THEN 'INCAPACIDAD' WHEN tpr.tprnombre = 'VACACIONES' THEN 'VACACIONES' END as title, s.slnnombre as salon, t.trndesde, t.trncolor as backgroundColor, CONCAT(t.trnnombre, ' DE: ', DATE_FORMAT(t.trndesde, '%H:%i'), ' A: ', DATE_FORMAT(t.trnhasta, '%H:%i'), ' ALM: ', DATE_FORMAT(t.trninicioalmuerzo, '%H:%i'), ' - ', DATE_FORMAT(t.trnfinalmuerzo, '%H:%i')) AS turno FROM `btyprogramacion_colaboradores` p INNER JOIN btyturno t ON t.trncodigo = p.trncodigo INNER JOIN btyturno_salon ts ON ts.trncodigo = t.trncodigo AND p.slncodigo = ts.slncodigo INNER JOIN btysalon s on s.slncodigo = ts.slncodigo INNER JOIN btycolaborador c ON c.clbcodigo = p.clbcodigo and c.clbcodigo = $codigo INNER JOIN btytercero trc ON c.trcdocumento = trc.trcdocumento JOIN btytipo_programacion tpr ON p.tprcodigo=tpr.tprcodigo";
