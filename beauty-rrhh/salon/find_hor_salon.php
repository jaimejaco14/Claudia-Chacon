...
if ($_POST['sln'] != "") {
    ...
    $sql = "SELECT h.horcodigo, hs.slncodigo, h.hordia, DATE_FORMAT(h.hordesde, '%H:%i') AS desde, DATE_FORMAT(h.horhasta, '%H:%i') AS hasta FROM btyhorario AS h INNER JOIN btyhorario_salon AS hs ON hs.horcodigo = h.horcodigo AND hs.slncodigo = '$salon' where hs.slhfechafin is null ORDER BY FIELD(h.hordia,'LUNES','MARTES','MIERCOLES','JUEVES','VIERNES','SABADO','DOMINGO','FESTIVO')";
?>
