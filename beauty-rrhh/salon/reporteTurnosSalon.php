<?php 
	...
	{   
		...
		$diasem="SELECT distinct(h.hordia) as dia, CONCAT(' DE: ', DATE_FORMAT(h.hordesde, '%H:%i'), ' A: ', DATE_FORMAT( h.horhasta , '%H:%i')) AS hor
                     ...
                     where hs.slncodigo='$codsalon' and hs.horcodigo=h.horcodigo and h.horestado='1'
            ...
            while ($data = $res->fetch_assoc()) { 
            	$dia=$data['dia'];
            	$col=$data['dia']." ".$data['hor'];
            	$EncabezadoColumnas=array(utf8_decode($col));

			    $AnchoColumnas=array(200);
				 
			    $AlineacionColumnas=array('L');

            	 $sql = mysqli_query($conn, "SELECT CONCAT(t.trnnombre, ' DE: ', DATE_FORMAT(t.trndesde, '%H:%i'), ' A: ', DATE_FORMAT(t.trnhasta, '%H:%i'), ' ALM: ', DATE_FORMAT(t.trninicioalmuerzo, '%H:%i'), ' - ', DATE_FORMAT(t.trnfinalmuerzo, '%H:%i')) AS turno 
					FROM btyturno AS t, btyhorario_salon AS hs, btyhorario as h, btyturno_salon as ts, btysalon as s 
					WHERE h.hordia='$dia' and h.horestado='1' and t.trnestado='1' and h.horcodigo=hs.horcodigo and s.slncodigo=hs.slncodigo and t.trnestado = 1 and ts.trncodigo=t.trncodigo and ts.horcodigo=h.horcodigo and s.slncodigo=ts.slncodigo and s.slncodigo='".$codsalon."' ORDER BY h.hordia, turno");
	...
	else
	{
        ...
        while ($data = $res->fetch_assoc()) { 
				...
				$sql = "SELECT CONCAT(t.trnnombre, ' DE: ', DATE_FORMAT(t.trndesde, '%H:%i'), ' A: ', DATE_FORMAT(t.trnhasta, '%H:%i'), ' ALM: ', DATE_FORMAT(t.trninicioalmuerzo, '%H:%i'), ' - ', DATE_FORMAT(t.trnfinalmuerzo, '%H:%i')) AS turno 
					FROM btyturno AS t, btyhorario_salon AS hs, btyhorario as h, btyturno_salon as ts, btysalon as s 
					WHERE h.hordia='$dia' and h.horestado='1' and t.trnestado='1' and h.horcodigo=hs.horcodigo and s.slncodigo=hs.slncodigo and t.trnestado = 1 and ts.trncodigo=t.trncodigo and ts.horcodigo=h.horcodigo and s.slncodigo=ts.slncodigo and s.slncodigo='".$codsalon."' ORDER BY h.hordia, turno";
					$res2=$conn->query($sql);
					...
					$reporteExcel->setActiveSheetIndex(0)->setCellValue("A".$i," ");
					$i++;
		}		
