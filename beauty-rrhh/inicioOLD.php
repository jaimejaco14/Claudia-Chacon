        ...
        <ul class="nav" id="side-menu">
            ...
            <li id="COLABORADORES">
                ...
                <ul class="nav nav-second-level">
                    ...
                    <li id="PERMISOS"><a href="colaborador/permisos.php"> <span class="nav-label">PERMISOS</span> </a></li>
...
<div class="container-fluid">
    <div class="content animate-panel">
                ...
                <div class="hpanel hbgyellow">
                    <div class="panel-body" id="detalleper" style="cursor:pointer;">
                        <div class="text-center">
                            ...
                            <small>
                                Permisos pendientes por gestionar
                            </small>
                        </div>
                    </div>
                </div>
                ...
                <div class="hpanel hbggreen">
                    <div class="panel-body" style="background-color: #62cb31!important">
                        <div class="text-center">
                            ...
                            <p class="text-big font-light">
                            <?php
                                $sqlcont= mysqli_fetch_array(mysqli_query($conn,"SELECT COUNT(*) FROM btyasistencia_procesada ap WHERE ap.prgfecha=date_sub(curdate(),interval 1 day) and ap.aptcodigo not in(1,5)"));
                                echo $sqlcont[0];
                            ?>
                            </p>
                            <small>
                                Novedades del dia anterior
                            </small>
                        </div>
                    </div>
                </div>
                ...
                <div class="hpanel">
                    <div class="panel-body">
                        <div class="text-center">
                            ...
                            <a href="colaborador/permisos.php"><button class="btn btn-warning btn-sm"  id="btnVerPermisos">Ir a Permisos</button></a><br><br>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row">
            <?php
                $sqlmesact="SELECT t.trcrazonsocial, sum(ap.apcvalorizacion) a
                            from btyasistencia_procesada ap
                            join btycolaborador col on col.clbcodigo=ap.clbcodigo
                            join btytercero t on t.trcdocumento=col.trcdocumento
                            where ap.aptcodigo <> 1 
                            and ap.prgfecha between (curdate() - INTERVAL DAYOFMONTH(curdate()) - 1 DAY) and curdate()
                            group by ap.clbcodigo,t.trcrazonsocial
                            order by a desc
                            limit 10";
                $sqldiant="SELECT t.trcrazonsocial, SUM(ap.apcvalorizacion) a
                            FROM btyasistencia_procesada ap
                            JOIN btycolaborador col ON col.clbcodigo=ap.clbcodigo
                            JOIN btytercero t ON t.trcdocumento=col.trcdocumento
                            WHERE ap.aptcodigo <> 1 AND ap.prgfecha = date_sub(curdate(),interval 1 day)
                            GROUP BY ap.clbcodigo,t.trcrazonsocial
                            ORDER BY a DESC
                            LIMIT 10";
                $sqlmesant="SELECT t.trcrazonsocial, SUM(ap.apcvalorizacion) a
                            FROM btyasistencia_procesada ap
                            JOIN btycolaborador col ON col.clbcodigo=ap.clbcodigo
                            JOIN btytercero t ON t.trcdocumento=col.trcdocumento
                            WHERE ap.aptcodigo <> 1 AND ap.prgfecha BETWEEN ((CURDATE() - INTERVAL 1 MONTH) - INTERVAL DAYOFMONTH(CURDATE()) - 1 DAY) AND LAST_DAY(CURDATE() - INTERVAL 1 MONTH)
                            GROUP BY ap.clbcodigo,t.trcrazonsocial
                            ORDER BY a DESC
                            LIMIT 10";
                $sqlanio="SELECT t.trcrazonsocial, SUM(ap.apcvalorizacion) a
                            FROM btyasistencia_procesada ap
                            JOIN btycolaborador col ON col.clbcodigo=ap.clbcodigo
                            JOIN btytercero t ON t.trcdocumento=col.trcdocumento
                            WHERE ap.aptcodigo <> 1 AND year(ap.prgfecha)=year(curdate())
                            GROUP BY ap.clbcodigo,t.trcrazonsocial
                            ORDER BY a DESC
                            LIMIT 10";
                $resdiant=$conn->query($sqldiant);
                while($rdiant=$resdiant->fetch_array()){
                    $coldiant.=utf8_encode($rdiant[0])."•";
                    $candiant.=$rdiant[1]."•";
                }
                 $resmeant=$conn->query($sqlmesant);
                while($mdiant=$resmeant->fetch_array()){
                    $colmesant.=utf8_encode($mdiant[0])."•";
                    $canmesant.=$mdiant[1]."•";
                }
                 $resmeact=$conn->query($sqlmesact);
                while($rmesact=$resmeact->fetch_array()){
                    $colmesact.=utf8_encode($rmesact[0])."•";
                    $canmesact.=$rmesact[1]."•";
                }
                $resanio=$conn->query($sqlanio);
                while($ranio=$resanio->fetch_array()){
                    $colanio.=utf8_encode($ranio[0])."•";
                    $cananio.=$ranio[1]."•";
                }
            ?>
            <div class="col-md-6">
                <div class="hpanel">
                    <div class="panel-body">
                    <center><b>TOP 10 VALORIZACION POR COLABORADOR</b></center>
                    <center>
                        <select class="form-control" name="selvaloriza" id="selvaloriza">
            ...
            <?php
                $sqlslnnom = mysqli_fetch_array(mysqli_query($conn,"SELECT group_concat(slnnombre order by slncodigo SEPARATOR '•') from btysalon where slnestado=1"));
                $sqlslncantdia = mysqli_fetch_array(mysqli_query($conn,"SELECT concat(
                                        IFNULL(sum(case when sl.slncodigo = 1 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 2 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 3 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 4 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 5 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 6 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 7 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 8 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 9 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 10 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 11 then 1 else 0 end),'0'))
                                        FROM btyasistencia_procesada ap
                                        join btysalon sl on sl.slncodigo=ap.slncodigo
                                        WHERE ap.prgfecha=date_sub(curdate(),interval 1 day)
                                        and ap.aptcodigo <> 1
                                        order by sl.slnnombre"));
                $sqlslncantmes = mysqli_fetch_array(mysqli_query($conn,"SELECT concat(
                                        IFNULL(sum(case when sl.slncodigo = 1 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 2 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 3 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 4 then 1 else 0 end),'0'),'•',
                                        ...
                                        FROM btyasistencia_procesada ap
                                        join btysalon sl on sl.slncodigo=ap.slncodigo
                                        WHERE ap.prgfecha between (curdate() - INTERVAL DAYOFMONTH(curdate()) - 1 DAY) and curdate()
                                        and ap.aptcodigo <> 1
                                        order by sl.slnnombre"));
                $sqlslncantmesa = mysqli_fetch_array(mysqli_query($conn,"SELECT concat(
                                        IFNULL(sum(case when sl.slncodigo = 1 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 2 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 3 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 4 then 1 else 0 end),'0'),'•',
                                        ...
                                        FROM btyasistencia_procesada ap
                                        join btysalon sl on sl.slncodigo=ap.slncodigo
                                        WHERE ap.prgfecha between ((curdate() - interval 1 month) - interval DAYOFMONTH(curdate()) - 1 DAY) and LAST_DAY(curdate() - interval 1 month)
                                        and ap.aptcodigo <> 1
                                        order by sl.slnnombre"));
                $sqlslncantanio = mysqli_fetch_array(mysqli_query($conn,"SELECT concat(
                                        IFNULL(sum(case when sl.slncodigo = 1 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 2 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 3 then 1 else 0 end),'0'),'•',
                                        IFNULL(sum(case when sl.slncodigo = 4 then 1 else 0 end),'0'),'•',
                                        ...
                                        FROM btyasistencia_procesada ap
                                        join btysalon sl on sl.slncodigo=ap.slncodigo
                                        WHERE year(ap.prgfecha)=year(curdate())
                                        and ap.aptcodigo <> 1
                                        order by sl.slnnombre"));
            ?>
            <div class="col-md-6">
                <div class="hpanel">
                    <div class="panel-body">
                    <center><b>NOVEDADES BIOMÉTRICO POR SALON</b></center>
                    <center>
                        <select class="form-control" name="graphmes" id="graphmes">
                            <option value="1">Dia Anterior</option>
                            <option value="2">Mes Actual</option>
                            <option value="3">Mes Anterior</option>
                            <option value="4">Año actual</option>
                ...
                <table class="table table-hover table-condensed">
                    ...
                    <?php
                        $sqlcumpmes="SELECT distinct(t.trcrazonsocial),day(c.clbfechanacimiento) as dia,sl.slnnombre
                                    ...
                                    WHERE MONTH(c.clbfechanacimiento )= MONTH(CURDATE())+1
                                    AND bty_fnc_estado_colaborador(c.clbcodigo)='vinculado'
...
<div class="modal fade" id="modal-detpermiso">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title"><i class="pe-7s-coffee"></i> PERMISOS PENDIENTES POR SALÓN</h4>
            </div>
            <div class="modal-body">
...
<script>//script grafico de torta
    ...
    var sln='<?php echo $sqlslnnom[0];?>';
    var candia='<?php echo $sqlslncantdia[0];?>';
    var canmes='<?php echo $sqlslncantmes[0];?>';
    var canmesa='<?php echo $sqlslncantmesa[0];?>';
    var cananio='<?php echo $sqlslncantanio[0];?>';
    graficar(candia);

    var bcoldia='<?php echo $coldiant;?>';
    var bcandia='<?php echo $candiant;?>';
    var bcolmesan='<?php echo $colmesant;?>';
    var bcanmesan='<?php echo $canmesant;?>';
    var bcolmesac='<?php echo $colmesact;?>';
    var bcanmesac='<?php echo $canmesact;?>';
    var bcolanio='<?php echo $colanio;?>';
    var bcananio='<?php echo $cananio;?>';
    graficar2(bcoldia,bcandia);

   $("#graphmes").change(function(e){
        var opc=$(this).val();
        clear();
        switch(opc){
            case '1':
   ...
   $("#selvaloriza").change(function(e){
        ...
        switch(opc){
            ...
            break;
            case '2':
                graficar2(bcolmesac,bcanmesac);
            break;
            case '3':
                graficar2(bcolmesan,bcanmesan);
            break;
            case '4':
                graficar2(bcolanio,bcananio);
            break;
        }
   });
    ...
    function graficar(can) {
        ...
        col = ["#cc0d01", "#fb6300","#fb9b00","#f8cf01","#f4fb00", "#addb08", "#04cf14", "#0d8ccb", "#0d51ce", "#290ccd", "#bb00cc"];
         ...
         for (i=0;i<nsln.length;i++)
        {
            salon.push(nsln[i]);
            cant.push(parseInt(csln[i]));
            col.push(col[i]);
        }
    ...
    function graficar2(col, can) {
        ...
        col = ["#cc0d01", "#fb6300","#fb9b00","#f8cf01","#f4fb00", "#addb08", "#04cf14", "#0d8ccb", "#0d51ce", "#290ccd", "#bb00cc"];
         ...
         for (i=0;i<vcol.length-1;i++)
        {
            colab.push(vcol[i]);
            cant.push(parseInt(vcant[i]));
            col.push(col[i]);
        }
            ...
            options: 
            {
                ...
                scales: {
                    xAxes: [{
                        ...
                        scaleLabel: {
                            display: true,
                            labelString: ' Cifras expresadas en miles de pesos ($)'
                        }
                    }]
                }       
            }
    ...
    {
        $("#canvasdiv2").html('');
        $("#canvasdiv2").html('<canvas id="myChart2" width="400" height="170"></canvas>');
    }  
</script>
