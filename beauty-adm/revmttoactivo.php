...
switch($opc){
	...
	case 'addrevmtto':
		...
		if($rowcons[1]>0){
			$sql="INSERT INTO btyactivo_programacion (pgmconsecutivo,actcodigo,pgmtipo,pgmfecha,pgmestado) 
			...
			if($conn->query($sql)){
				...
				if($_POST['automtto']){
					...
					while($sw){
						...
						//echo $sqlautomtto;
						if($fecha!=0){
							$sqlinsmtto="INSERT IGNORE INTO btyactivo_programacion (pgmconsecutivo,actcodigo,pgmtipo,pgmfecha,pgmestado) 
								VALUES ((SELECT if(MAX(c.pgmconsecutivo) is null,1,MAX(c.pgmconsecutivo)+1) from btyactivo_programacion as c),
		...
		if($rowcons[0]>0){
			$sql2="INSERT INTO btyactivo_programacion (pgmconsecutivo,actcodigo,pgmtipo,pgmfecha,pgmestado) 
				VALUES ((SELECT if(MAX(c.pgmconsecutivo) is null,1,MAX(c.pgmconsecutivo)+1) from btyactivo_programacion as c),
				...
				if($_POST['autorev']){
					...
					while($sw){
						...
						if($fecha!=0){
							...
							//echo $bool[0];
							if($bool[0]==0){
								$sqlinsrev="INSERT IGNORE INTO btyactivo_programacion (pgmconsecutivo,actcodigo,pgmtipo,pgmfecha,pgmestado) 
									VALUES ((SELECT if(MAX(c.pgmconsecutivo) is null,1,MAX(c.pgmconsecutivo)+1) from btyactivo_programacion as c),
