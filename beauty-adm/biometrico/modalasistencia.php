...
switch($oper){
	case 1:
		...
		$sql="SELECT ap.prgfecha, tn.trndesde AS horario, ab.abmnuevotipo, ab.abmhora, apt.aptnombre AS estado
				...
				WHERE MONTH(ap.prgfecha)=$mes AND ap.clbcodigo=$id AND ab.abmnuevotipo='ENTRADA' AND apt.aptcodigo=2 order by ap.prgfecha asc";
	break;
	case 2:
		$head=$head1;
		$sql="SELECT ap.prgfecha, tn.trnhasta AS horario, ab.abmnuevotipo, ab.abmhora, apt.aptnombre AS estado
				...
				JOIN btyasistencia_biometrico ab ON ab.abmcodigo=ap.abmcodigo
				JOIN btyasistencia_procesada_tipo apt ON apt.aptcodigo=ap.aptcodigo
				WHERE MONTH(ap.prgfecha)=$mes AND ap.clbcodigo=$id AND ab.abmnuevotipo='SALIDA' AND apt.aptcodigo=3 order by ap.prgfecha asc";
	break;
	case 3:
		$col3="display:none;";
		$head=$head1;
		$sql="SELECT ap.prgfecha, concat(tn.trndesde,' a ',tn.trnhasta) AS horario, IFNULL(NULL, IF((
				...
				JOIN btyturno tn ON tn.trncodigo=ap.trncodigo
				JOIN btyasistencia_procesada_tipo apt ON apt.aptcodigo=ap.aptcodigo
				WHERE MONTH(ap.prgfecha)=$mes AND ap.clbcodigo=$id AND ap.abmcodigo IS NULL and ap.aptcodigo=6 order by ap.prgfecha asc";
	break;
	case 4:
		$col2="display:none;";
		$col3="display:none;";
		...
		$sql="SELECT ap.prgfecha, concat(tn.trndesde,' a ',tn.trnhasta) AS horario, null,null, apt.aptnombre AS estado
				...
				JOIN btyturno tn ON tn.trncodigo=ap.trncodigo
				JOIN btyasistencia_procesada_tipo apt ON apt.aptcodigo=ap.aptcodigo
				WHERE MONTH(ap.prgfecha)=$mes AND ap.clbcodigo=$id AND ap.abmcodigo IS NULL and ap.aptcodigo=4 order by ap.prgfecha asc";
	break;
	case 5:
		$head=$head2;
	...
	case 6:
		...
		$sql="SELECT DISTINCT(ap.prgfecha), null AS horario, NULL, NULL, apt.aptnombre AS estado
				...
				JOIN btyasistencia_biometrico ab ON ab.abmcodigo=ap.abmcodigo
				JOIN btyasistencia_procesada_tipo apt ON apt.aptcodigo=ap.aptcodigo
				WHERE MONTH(ap.prgfecha)=$mes AND ap.clbcodigo=$id AND apt.aptcodigo=5";
	break;
}
