...
include '../../cnx_data.php';
  ...
  if (is_array($detallesBiometrico) || is_object($detallesBiometrico))
  {
          ...
          if($tam==$cont){
            echo json_encode(array("RES"=>"TRUE","TINS"=>$cont,"TAM"=>$tam));
          }else if(($cont>0)||($actu>0)){
            echo json_encode(array("RES"=>"FALSE","ACTU"=>$actu,"TAM"=>$tam,"ERR"=>$error,"TINS"=>$cont,"VECERR"=>$vector));
          }else if(($cont==0)&&($actu==0)){
            echo json_encode(array("RES"=>"ERROR","ACTU"=>$actu,"TAM"=>$tam,"ERR"=>$error,"TINS"=>$cont));
          }
          //control de logs
