...
$mail = new PHPMailer;
   ...
   $body='Se adjuntan reporte(s) los servicios ofrecidos por Beauty Soft ERP.<br> <br><br> Se filtro por la siguiente cadena: <br><br>'.$_SESSION['vrbrptsrvc'];
   ...
   for ($i=0; $i<5 ; $i++) { 
    ...
    $mail->Subject = 'Reporte  de Servicio';
    ...
    //atyachment funciona como adjuntador de registros, usando el fopen la funcion mailer facilita la accion y solo se usa una funcion para adjuntar, acompañado de la ruta del archivo
      $mail->addAttachment("tmp/Reporte de Servicios ".$_SESSION['user_session'].".pdf");
      $mail->addAttachment("tmp/Reporte de Servicios ".$_SESSION['user_session'].".xls");
...
if (!$mail->send()) {
        echo "Mailer Error: " . $mail->ErrorInfo;
         unlink("tmp/Reporte de Servicios ".$_SESSION['user_session'].".pdf");
         unlink("tmp/Reporte de Servicios ".$_SESSION['user_session'].".xls");
    } else {
