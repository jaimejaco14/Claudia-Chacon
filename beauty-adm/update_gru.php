...
if ($_POST['cod'] != "") {
	...
	if ( "" != $_FILES['upImagen']['name']){
        ...
        $sql = "UPDATE `btygrupo` SET `tpocodigo`=$sbl,`grunombre`='$nombre',`grualias`='$alias',`grudescripcion`='$desc',`gruimagen`='$img_name' WHERE grucodigo = $codigo";
} else {
	$sql = "UPDATE `btygrupo` SET `tpocodigo`=$sbl,`grunombre`='$nombre',`grualias`='$alias',`grudescripcion`='$desc' WHERE grucodigo = $codigo";
}
if ($conn->query($sql)) {
	echo "TRUE";
}
