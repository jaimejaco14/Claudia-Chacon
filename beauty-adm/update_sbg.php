...
if ($_POST['cod'] != "") {
	...
	if ( "" != $_FILES['upImagen']['name']){
        ...
        $sql = "UPDATE `btysubgrupo` SET `grucodigo`=$sbl,`sbgnombre`='$nombre',`sbgalias`='$alias',`sbgdescripcion`='$desc',`sbgimagen`='$img_name' WHERE  sbgcodigo = $codigo";
} else {
	$sql = "UPDATE `btysubgrupo` SET `grucodigo`=$sbl,`sbgnombre`='$nombre',`sbgalias`='$alias',`sbgdescripcion`='$desc' WHERE  sbgcodigo = $codigo";
}
if ($conn->query($sql)) {
	echo "TRUE";
}
