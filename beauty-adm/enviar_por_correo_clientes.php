...
$mail = new PHPMailer;
   ...
   $body='Se adjuntan reporte(s) de los clientes contenidos en Beauty Soft ERP.<br> <br> <br>  Se filtro por la siguiente cadena: <br><br>'.$_SESSION['vrbrpc'];
   ...
   for ($i=0; $i<5 ; $i++) { 
    ...
    //atyachment funciona como adjuntador de registros, usando el fopen la funcion mailer facilita la accion y solo se usa una funcion para adjuntar, acompañado de la ruta del archivo
      $mail->addAttachment("tmp/Reporte de Clientes ".$_SESSION['user_session'].".xls");
      $mail->addAttachment("tmp/Reporte de clientes ".$_SESSION['user_session'].".pdf");
...
if (!$mail->send()) {
        echo "Mailer Error: " . $mail->ErrorInfo;
         unlink("tmp/Reporte de Clientes ".$_SESSION['user_session'].".xls");
         unlink("tmp/Reporte de clientes ".$_SESSION['user_session'].".pdf");
    } else {
