		    	...
		    	if(fhas >= fdes){
		    		if((lin!=0)||(gru!=0)||(subg!=0)||(cara!=0)){
		    			if(trp!=null){
					    	...
					    	colusalon.push({data:'gru',title:'GRUPO'});
					    	colusalon.push({data:'sgru',title:'SUBGRUPO'});
					    	colusalon.push({data:'car',title:'CARACTERISTICA'});
					    	colusalon.push({data:'und',title:'UND'});
					    	$("#tbrepadmi tfoot tr").append('<th class="csstot"></th><th class="csstot"></th><th class="csstot"></th><th class="csstot"></th><th class="csstot"></th><th class=" csstot2 text-right">TOTAL</th>');
					    	var sw='';
					    	var allsln='';
					    	if(sln==null){
					    	...
					    	var listado = $('#tbrepadmi').DataTable({
								...
								"columnDefs":[
									...
									{className:"text-center aaa","targets":[2,3,4,5]},
					...
					footer: true,
						exportOptions: {
						    ...
						    format: {
						        ...
						        footer:function(data, row, column, node){
						        	ret = data.substring(0,1)=='<'?data.split('>')[1].split('<')[0].replace(/\$|,/g, ''):						                        		data.replace(/\$|,/g, '');
						        	var ret2 = isNaN(ret)?ret:parseFloat(ret);
						        	return ret2;
						        }
						    }
						}
