...
echo "estado del whitch :".$_SESSION['sw1'];
	...
	$cod=$cod+1;
	$_SESSION['num_sesion']=$cod;
}
if ($sw==1) {
		echo "<br>";
		echo $codee= $_SESSION["codigoUsuario"];
		echo "<br>";
		echo $nomb_us=$_SESSION['user_session'];
		echo "<br>";
...
$sql="INSERT INTO `btysesiones`(`sescodigo`, `sesmodulo`, `usucodigo`, `seslogin`, `sesdireccionipv4wan`, `sestipodispotivo`, `sesnavegador`, `sesfechainicio`, `seshorainicio`, `sesfechafin`, `seshorafin`, `sesfallida`, `sesestado`) VALUES ($cod, 'ADMINISTRATIVO', $codee,'$nomb_us','$ipwan','$sist','$brow','$sesinf','$sesinh','$sesfinf','$sesfinh',$fallo,$est)";
if ($conn->query($sql)) {
		echo "TRUE";
