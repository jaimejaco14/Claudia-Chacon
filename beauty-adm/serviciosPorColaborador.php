...
<table id="example2" class="table table-striped table-bordered table-hover">
    <thead>
        <tr>
            <th><center><i class="fa fa-close text-danger"></i></center></th>
            <th>NOMBRE</th>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        echo "<tr>
            <td><button class='btn btn-sm btn-default' title='Revocar Servicio' type='button' onclick='remover_servicio(".$row['sercodigo'].", ".$codigoColaborador.", this)'><i class='pe-7s-trash text-info'></i></button></td>
...
<?php
  if ($total_paginas > 1) {
                        ...
                        echo '<ul class="pagination pull-right"></ul>';
                            if ($pageNum != 1) {
                                echo '<li><a class="paginate" onclick="paginar_servicios('.($pageNum-1).', '.$codigoColaborador.');" data="'.($pageNum-1).'">Anterior</a></li>';
                            }
                                for ($i=1;$i<=$total_paginas;$i++) {
                                    if ($pageNum == $i) {
                                        //si muestro el índice de la página actual, no coloco enlace
                                        echo '<li class="active"><a onclick="paginar_servicios('.$i.', '.$codigoColaborador.');">'.$i.'</a></li>';
                                    } else if ($pageNum > ($i + 2) or $pageNum < $i - 2) {
                                        //echo '<li hiddenn><a class="paginate" onclick="paginar_servicios('.$i.');" data="'.$i.'">'.$i.'</a></li>';

                                    } else {
                                        //si el índice no corresponde con la página mostrada actualmente,
                                        //coloco el enlace para ir a esa página
                                        echo '<li><a class="paginate" onclick="paginar_servicios('.$i.', '.$codigoColaborador.');" data="'.$i.'">'.$i.'</a></li>';
                                    }
                                }
                            if ($pageNum != $total_paginas) {
                                echo '<li><a class="paginate" onclick="paginar_servicios('.($pageNum+1).', '.$codigoColaborador.');" data="'.($pageNum+1).'">Siguiente</a></li>';    
                            }
                       echo '</ul>';
