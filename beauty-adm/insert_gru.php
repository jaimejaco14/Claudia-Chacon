...
if ($_POST['nombre'] != "") {
	...
	$desc = $_POST['desc'];
	$gru = $_POST['tipo'];
	$max = "SELECT MAX(grucodigo) as m FROM `btygrupo`";
	...
	$sql = "INSERT INTO `btygrupo`(`grucodigo`, `tpocodigo`, `grunombre`, `grualias`, `grudescripcion`, `gruimagen`, `gruestado`) VALUES ($codigo, $gru, '$nombre', '$alias', '$desc','$img_name',1)";
	if ($conn->query($sql)) {
		echo "TRUE";
	}
}