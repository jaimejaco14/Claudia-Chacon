...
$result = $conn->query($query_num_col);
  ...
  $offset = ($pageNum - 1) * $rowsPerPage;
    ...
    $sql =  "SELECT distinct t.trcrazonsocial, c.trcdocumento, tdi.tdialias, c.cliempresa, o.ocunombre, b.brrnombre, c.clifecharegistro, c.cliimagen FROM btycliente c INNER JOIN btytercero t ON t.trcdocumento = c.trcdocumento INNER JOIN btyocupacion o on c.ocucodigo = o.ocucodigo INNER JOIN btybarrio b ON t.brrcodigo = b.brrcodigo INNER JOIN btytipodocumento tdi on c.tdicodigo = tdi.tdicodigo WHERE cliestado = 1 limit $offset, $rowsPerPage";
    $result = $conn->query($sql);

 if ($result->num_rows > 0) {
    //replicacion de etiqueta por replicacion de codigo :)
    echo ' <div class="row">
                        <div class="col-lg-12">  
    ...
    <a onclick="detalles('.$row['trcdocumento'].');">
        <div class="hpanel hgreen contact-panel">
            <div class="panel-body">
                '.$type.'
                <img alt="logo" class="img-circle m-b" src="imagenes/default.jpg">
                <h3>'.$nom.'</h3>
