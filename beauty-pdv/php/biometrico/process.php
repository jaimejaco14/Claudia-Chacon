<?php 
	//session_start();
  	...
  	$sql=mysqli_query($conn,"(SELECT a.clbcodigo, t.trcrazonsocial, crg.crgnombre, ctc.ctcnombre,b.aptnombre, CONCAT(TIME_FORMAT(j.trndesde, '%H:%i'), ' a ', 
			...
			WHERE MONTH(a.prgfecha) = MONTH(CURDATE()) AND NOT a.prgfecha = CURDATE() AND s.slncodigo ='".$_SESSION['PDVslncodigo']."' AND NOT b.aptfactor = '0.0000' AND a.apcobservacion = ''
	...
	UNION 
 	...
 	(SELECT a.clbcodigo, t.trcrazonsocial, crg.crgnombre, ctc.ctcnombre,b.aptnombre, 
		...
		a.slncodigo, a.apcobservacion, a.prgfecha, a.abmcodigo, a.aptcodigo, CONCAT('$',FORMAT(a.apcvalorizacion,0))AS apcvalorizacion
		FROM btyasistencia_procesada a
		JOIN btyasistencia_procesada_tipo b ON a.aptcodigo=b.aptcodigo
		JOIN btycolaborador c ON c.clbcodigo=a.clbcodigo
		JOIN btytercero t ON t.trcdocumento=c.trcdocumento
		...
		JOIN btycategoria_colaborador ctc ON ctc.ctccodigo=c.ctccodigo
		JOIN btyturno j ON j.trncodigo=a.trncodigo
		WHERE MONTH(a.prgfecha) = MONTH(CURDATE()) AND NOT a.prgfecha = CURDATE() AND a.abmcodigo IS NULL AND NOT a.aptcodigo=6 AND s.slncodigo = '".$_SESSION['PDVslncodigo']."' AND NOT b.aptfactor = '0.0000' AND a.apcobservacion = ''
		ORDER BY t.trcrazonsocial) 
	...
	(
		...
		AND a.aptcodigo=6 AND s.slncodigo = '".$_SESSION['PDVslncodigo']."' AND NOT b.aptfactor = '0.0000' AND a.apcobservacion = '')
