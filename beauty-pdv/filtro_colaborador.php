...
if ($id = $_POST['nombre']) {
    $sql = "SELECT t.trcrazonsocial, c.trcdocumento, tdi.tdialias, c.cliempresa, o.ocunombre, b.brrnombre, c.clifecharegistro, c.cliimagen FROM btycliente c INNER JOIN btytercero t ON t.trcdocumento = c.trcdocumento INNER JOIN btyocupacion o on c.ocucodigo = o.ocucodigo INNER JOIN btybarrio b ON t.brrcodigo = b.brrcodigo INNER JOIN btytipodocumento tdi on c.tdicodigo = tdi.tdicodigo WHERE (c.trcdocumento LIKE '".$id."%' or t.trcrazonsocial LIKE '%".$id."%') AND  clbestado = 1";
} else { 
    ...
    if ($_POST['genero'] == 'F' OR $_POST['genero'] == 'M') { 
        $sql = $sql." AND c.clisexo = '".$_POST['genero']."'";
    } 
    if ($_POST['cargo'] != 0) {
        $sql = $sql." AND c.ocucodigo = ".$_POST['cargo'];
    }
    if ($_POST['tipo'] == 'INTERNO' OR $_POST['tipo'] == 'EXTERNO') {
        $sql = $sql." AND c.clitiporegistro = '".$_POST['tipo']."'";
    }
    ...
    <a onclick="detalles('.$row['trcdocumento'].');">
        <div class="hpanel hgreen contact-panel">
            <div class="panel-body">
                '.$type.'
                <img alt="logo" class="img-circle m-b" src="../contenidos/imagenes/'.$src.'">
                <h3>'.$nom.'</h3>
