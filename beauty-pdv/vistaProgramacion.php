	...
	<div class="row">
			...
			<div class="hpanel">
				<div class="panel-heading">
					<div class="panel-tools">
						...
						<a href="aaprog.php" data-toggle="tooltip" data-placement="top" title="Ir a Crear programación" class="btn btn-info" style="color:white;"><i></i> Crear programación</a>
						...
						<button id="btnlog" class="btn btn-info" data-toggle="tooltip" data-placement="top" title="Muestra el registro de cambios en programación"><i class="fa fa-eye"></i> Consulta de Cambios</button>
					...
					echo " <h3>VISTA GENERAL DE PROGRAMACIÓN " . "<b>" . $fetch['slnnombre'] ."</b> MES DE <b>".   $mes.'-'.$vec[0]."</b></h3>";
				...
				<div class="panel-body">
					...
					<div class="table-responsive" id="divTabla">
						<table class="table table-hover table-striped table-bordered grilla">
							<thead>
								<tr>
			            ...
			            <tbody>
			            	...
			            	$sql = mysqli_query($conn, "SELECT DISTINCT a.clbcodigo, c.trcrazonsocial, crg.crgnombre, crg.crgcodigo FROM btyprogramacion_colaboradores a JOIN btycolaborador b ON a.clbcodigo=b.clbcodigo JOIN btytercero c ON c.tdicodigo=b.tdicodigo AND c.trcdocumento=b.trcdocumento JOIN btycargo crg ON crg.crgcodigo=b.crgcodigo WHERE MONTH(a.prgfecha) = MONTH('$fecha') AND YEAR(a.prgfecha) = YEAR('$fecha')  AND a.slncodigo = '".$_GET['slncodigo']."' ORDER BY crg.crgnombre,c.trcrazonsocial");


			            	while ($row = mysqli_fetch_array($sql)) 
			            	{
			            		echo '<tr class="crg'.$row['crgcodigo'].' resgrid" data-crg="'.$row['crgcodigo'].'">
			            		<td class="datacol" style="white-space: nowrap" data-clb="'.$row['clbcodigo'].'">'.utf8_encode($row['trcrazonsocial']).' [<b>'.$row['crgnombre'].'</b>]</td>';
	    		...
	    		<div class="modal-content">
	    			...
	    			<div class="modal-body">
	    				...
	    				<div class="form-group">
	    					...
	    					<input id="mdfecha" class="form-control" placeholder="Click para seleccionar fecha..." readonly="">
	    				</div>
	    				<div class="form-group">
	    					<div class="row">
	    ...
	    <script>
	    	$(document).ready(function(){
				...
				var aniact=today.getFullYear();
				if(mesactual==mes){
					var dia=today.getDate();
				}else if((mes<mesactual) && (yr<=aniact)){
					$(".edtprog").attr('disabled', 'disabled');
					$(".edtprog").removeAttr('href');
				}else{
					var dia=1;
				}
				$("#mdfecha").datepicker('setStartDate', yr+'-'+mes+'-'+dia);
				$("#mdfecha").datepicker('setEndDate', yr+'-'+mes+'-'+udm);
	    	});
	    				...
	    				$.ajax({
	    					...
	    					success:function(res){
	    						if(res==1){
	    							...
	    							swal('No se puede modficar','El colaborador seleccionado NO TIENE programación el día seleccionado o está asignado a otro salón este día.','warning');
	    						}else if(res==3){
