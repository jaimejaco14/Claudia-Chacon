 <?php 
include 'head.php';
$sesuser=$_SESSION['PDVcodigoUsuario'];
if($sesuser!=1){
	$sql="SELECT COUNT(*) FROM btyusuario u JOIN btycolaborador c on c.trcdocumento=u.trcdocumento where c.crgcodigo in (5,6) and u.usucodigo=$sesuser";
	$res=$conn->query($sql);
	$row=$res->fetch_array();
	$swtc=$row[0];
}else{
	...
	<div class="row">
		<div class="col-md-2">
			<div class="input-group">
				...
				<input id="periodo" class="form-control" placeholder="Elija periodo...">
			</div>
		</div>
		<div class="col-md-3">
			<div class="input-group">
		...
		<div class="col-md-3">
			<a href="" class="btn btn-default" data-toggle="tooltip" title="Borrar todo / volver a iniciar"><span class="fa fa-refresh text-info"></span> Reiniciar</a>
			<button id="insert" class="btn btn-warning hidden" data-toggle="tooltip" title="Guardar la programación"><span class="fa fa-upload text-default"></span><b> Insertar</b></button>
		</div>
	</div><br>
	<div class="row">
		<div class="table-responsive">
			<table id="tbprog" class="table table-bordered table-striped">
				...
				<tfoot>
	         		<tr class="colxdia apertura hidden">
	         			<th colspan="2">Apertura</th>
	         		</tr>
	         		<tr class="colxdia otrotrn hidden">
	         			<th colspan="2">Turnos Intermedios</th>
	         		</tr>
	         		<tr class="colxdia cierre hidden">
...
<div class="modal fade" id="modal-prevmonth">
	...
	<div class="content" style="background-color: white;">
		<div class="row">
			<h1 class="text-center">Usted no tiene autorización para ingresar a este módulo.<br>Comuníquese con el departamento de sistemas para solicitar acceso.</h1>
		</div>
	</div>
	<?php
}
...
<script src="../lib/vendor/selectpicker/selectpicker.js"></script>
...
<script type="text/javascript">
    ...
    $("#periodo").change(function(e){
    	if(e.handled !== true)
        {
	    	...
	    	$("#copyto").datepicker('setEndDate', yr+'-'+mes+'-'+udm);
    	...
    	if(sw==0){
	    	...
	    	$("#tbprog tbody").append('<tr id="th"><th class="text-center">Colaborador</th><th>Puesto</th>');
	     	for(var i=1;i<=cdia;i++){
	     		...
	     		$(".apertura").append('<th class="text-center text-danger cxdape'+i+'">0</th>');
	     		$(".cierre").append('<th class="text-center text-danger cxdcierr'+i+'">0</th>');
	     		$(".otrotrn").append('<th class="text-center text-danger cxdotro'+i+'">0</th>');
    ...
    $('#slcolab').on('show.bs.select', function (e) {
        $('.bs-searchbox').addClass('algo44');
        $('.algo44 .form-control').attr('id', 'fucker44');
        if($("#slcargo").val().length>0){
    		$("#slcargo").attr('disabled','true');
        }
    });
    $(document).on('keyup', '#fucker44', function(event) {
        var seek = $(this).val();
        var crg=$("#slcargo").val();
        ...
        $.ajax({
            ...
            type: 'POST',
            data:'opc=clb&txt='+seek+'&crg='+crg+'&sln='+sln,
	...
	function horariosalon(){
		var sln=$("#cod_salon").val();
		$.ajax({
			url:'zprog/fillcont.php',
			type:'POST',
			data:{opc:'horsln', sln:sln},
			success:function(res){
				JSONHSALON=JSON.parse(res);
			}
		})
	}
		...
		for(i=1;i<=cdia;i++){
			...
			$(".td-dia"+i).each(function(e){
				if($(this).data('tilab') == 1){
					...
					if(tdia=='ds'){
						var tdesde=JSONDS.filter(x => (x.codigo==trn))[0].tdesde;
						var thasta=JSONDS.filter(x => (x.codigo==trn))[0].thasta;
						var hdesde=JSONHSALON.filter(y => y.dia=='LUN')[0].hdesde;
						var hhasta=JSONHSALON.filter(y => y.dia=='LUN')[0].hhasta;
					}else if(tdia=='sb'){
						var tdesde=JSONSAB.filter(x => (x.codigo==trn))[0].tdesde;
						var thasta=JSONSAB.filter(x => (x.codigo==trn))[0].thasta;
						var hdesde=JSONHSALON.filter(y => y.dia=='SAB')[0].hdesde;
						var hhasta=JSONHSALON.filter(y => y.dia=='SAB')[0].hhasta;
					}else if(tdia=='do'){
						var tdesde=JSONDOM.filter(x => (x.codigo==trn))[0].tdesde;
						var thasta=JSONDOM.filter(x => (x.codigo==trn))[0].thasta;
						var hdesde=JSONHSALON.filter(y => y.dia=='DOM')[0].hdesde;
						var hhasta=JSONHSALON.filter(y => y.dia=='DOM')[0].hhasta;
					}else if(tdia=='fs'){
						var tdesde=JSONFES.filter(x => (x.codigo==trn))[0].tdesde;
						var thasta=JSONFES.filter(x => (x.codigo==trn))[0].thasta;
						var hdesde=JSONHSALON.filter(y => y.dia=='FES')[0].hdesde;
						var hhasta=JSONHSALON.filter(y => y.dia=='FES')[0].hhasta;
					}else if(tdia=='es'){
						var tdesde=JSONESP.filter(x => (x.codigo==trn))[0].tdesde;
						var thasta=JSONESP.filter(x => (x.codigo==trn))[0].thasta;
						var hdesde=JSONHSALON.filter(y => y.dia=='ESP')[0].hdesde;
						var hhasta=JSONHSALON.filter(y => y.dia=='ESP')[0].hhasta;
					}


					if(tdesde==hdesde){
						acumape++;
					}else if(thasta==hhasta){
						acumcie++;
					}else{
						acumotro++;
					}
			...
			$(".cxdape"+i).empty();
			$(".cxdape"+i).html(acumape);
			$(".cxdcierr"+i).empty();
			$(".cxdcierr"+i).html(acumcie);
			$(".cxdotro"+i).empty();
			$(".cxdotro"+i).html(acumotro);
		}
	}
			...
			if ($('#chkcopy').is(':checked')) {
				...
				for (var i = 0; i < temp.length; ++i) {
					...
					if(ddd[1]==mes){
						...
						$('#'+row).find('.td-dia'+parseInt(ddd[2])).removeData('trn');
						$('#'+row).find('.td-dia'+parseInt(ddd[2])).attr('data-trn',null).attr('data-trn',trntmp[0].codigo);
	...
	$("#insert").click(function(e){
		...
		if(swpto!=0){
			if($(".asd").length>0){
				...
				swal('Programación vacía!','Por favor asigne programación a algún colaborador','error');
			}
		}else{
