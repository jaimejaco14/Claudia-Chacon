...
switch($opc){
	...
	break;
	case 'clb':
		//la funcion bty_fnc_salonbase_col filtra a los colaboradores que pertenecen al salon enviado; eliminar para quitar el filtro
		...
		if($crg=='adm'){
			$sql="SELECT c.clbcodigo, t.trcrazonsocial
					...
					WHERE t.trcrazonsocial LIKE '%".$txt."%' AND cg.crgincluircolaturnos=0 AND bty_fnc_estado_colaborador(c.clbcodigo)='VINCULADO' and bty_fnc_salonbase_col(c.clbcodigo)=$sln";
		}else{
			$sql="SELECT c.clbcodigo, t.trcrazonsocial
					FROM btycolaborador c
					JOIN btytercero t ON t.trcdocumento=c.trcdocumento
					WHERE t.trcrazonsocial like '%".$txt."%' and c.crgcodigo IN (".$crg.") and bty_fnc_estado_colaborador(c.clbcodigo)='VINCULADO' and bty_fnc_salonbase_col(c.clbcodigo)=$sln";
		}
		$res=$conn->query($sql);
		while($row=$res->fetch_array()){
